...
namespace Calcpad.Core
{
    internal class HtmlWriter : OutputWriter
    {
        ...
        internal override string FormatVector(Vector vector)
        {
            ...
            // Render as column vector using matrix format (with brackets)
            sb.AppendLine("<span class=\"matrix\">");
            for (int i = 0; i < len; ++i)
            {
                if (i == maxCount)
                {
                    var n = len - maxCount;
                    sb.AppendLine($"<span class=\"tr\"><span class=\"td\"></span><span class=\"td\" title=\"{n - Math.Sign(n - 1)} elements skipped.\">⋮</span><span class=\"td\"></span></span>");
                    i = len - 1;
                }
